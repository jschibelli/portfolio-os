...
try {
	...
	console.warn('[availability-api] googleapis not available');
	google = null;
	calendar = null;
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		...
		// Check if Google Calendar service account credentials are configured
		...
		const busyTimes =
			busyResponse.data.calendars?.[process.env.GOOGLE_CALENDAR_ID || 'primary']?.busy || [];
