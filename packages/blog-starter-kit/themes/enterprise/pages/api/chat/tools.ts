...
async function createGoogleAuthClient() {
	if (
		...
		console.log(
			'🔍 Debug: Need GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_OAUTH_REFRESH_TOKEN',
		);
		return null;
	}
	...
	try {
		...
		console.error(
			'🔍 Debug: Error creating OAuth client:',
...
export const CHAT_TOOLS = [
			...
			parameters: {
				...
				properties: {
					...
					requestedTime: {
						type: 'string',
						description:
							'Specific time the user wants (e.g., "2:00 PM", "3:00 PM", "10:00 AM" or ISO 8601 format). The system will look ahead 14 days to find the first available occurrence of this time. If the user asks for "2:00 PM", set this to "2:00 PM".',
					},
					preference: {
						type: 'string',
						description:
							'Time preference: "morning" (9 AM - 1 PM), "afternoon" (1 PM - 6 PM), or "any" (full day). Use this when user specifies a preference.',
	...
	},
	{
		...
		function: {
			...
			description:
				'Get a specific section from a case study to provide detailed insights about the project, technical implementation, challenges, and results',
			parameters: {
				type: 'object',
				properties: {
					caseStudyId: {
					...
					chapterId: {
						type: 'string',
						description:
							'ID of the section to retrieve (e.g., problem-statement, research-analysis, solution-design, implementation, results-metrics, lessons-learned, next-steps)',
	...
	},
	{
		...
		function: {
			...
			description:
				'Process a booking request - check for existing bookings first, then show booking modal if needed',
			parameters: {
				type: 'object',
				properties: {
					email: {
	...
	},
	{
		...
		function: {
			...
			description:
				'Show a unified booking modal that combines contact information collection and calendar selection',
			parameters: {
				type: 'object',
				properties: {
					timezone: {
					...
					preference: {
						type: 'string',
						description:
							'Time preference: "morning" (9 AM - 1 PM), "afternoon" (1 PM - 6 PM), or "any" (full day)',
						enum: ['morning', 'afternoon', 'any'],
						default: 'any',
					},
				},
	...
	},
	{
		...
		function: {
			...
			description:
				'Show a confirmation modal with booking details before creating the calendar event',
			parameters: {
				type: 'object',
				properties: {
					name: {
...
async function getAvailability(parameters: any) {
	...
	try {
		...
		if (isNextWeekRequest) {
			...
			console.log(
				'🔍 Debug: Next week request detected, looking from:',
		...
		if (requestedTime) {
			...
			if (!isNaN(requestedDate.getTime())) {
				...
				if (requestedDate > thirtyDaysFromNow) {
					...
					endDate = new Date(now.getTime() + lookaheadDays * 24 * 60 * 60 * 1000);
					console.log(
						'🔍 Debug: Specific time requested, looking ahead 2 weeks from now:',
						startDate.toISOString(),
						'to',
						endDate.toISOString(),
					);
			...
			endDate = new Date(now.getTime() + lookaheadDays * 24 * 60 * 60 * 1000);
			console.log(
				'🔍 Debug: General availability request, looking ahead 14 days from now:',
				startDate.toISOString(),
				'to',
				endDate.toISOString(),
			);
		...
		try {
			...
			console.log(
				'🔍 Debug: Failed to fetch busy times with calendar ID:',
				calendarId,
				'Error:',
				error,
			);
		...
		console.log(
			'🔍 Debug: Google Calendar API response:',
		...
		if (requestedTime) {
			...
			if (requestedHour !== 0 || requestedMinute !== 0) {
				...
				console.log(
					'🔍 Debug: Looking for slots at or after:',
				...
				availableSlots.forEach((slot, index) => {
					...
					const slotTimeInBusinessTZ = new Date(
						slotTime.toLocaleString('en-US', { timeZone: timezone }),
					);
					console.log(
						`🔍 Debug: Slot ${index}: ${slot.start} -> ${slotTimeInBusinessTZ.toLocaleString('en-US', { timeZone: timezone })} (${slotTimeInBusinessTZ.getHours()}:${slotTimeInBusinessTZ.getMinutes().toString().padStart(2, '0')})`,
					);
				});
				...
				const matchingSlot = availableSlots.find((slot) => {
					...
					if (requestedHour === 15 && requestedMinute === 0) {
						console.log(
							`🔍 Debug: 3:00 PM MATCH CHECK - Slot: ${slot.start} (${slotHour}:${slotMinute}) vs Requested: ${requestedHour}:${requestedMinute}`,
						);
					}
				...
				if (next) {
					...
					if (requestedHour === 15 && requestedMinute === 0) {
						console.log(
							`❌ Debug: NO 3:00 PM SLOT FOUND! Total available slots: ${availableSlots.length}`,
						);
						...
						availableSlots.forEach((slot, index) => {
							...
							console.log(
								`  ${index + 1}. ${slot.start} -> ${slotTimeInBusinessTZ.toLocaleString('en-US', { timeZone: timezone })}`,
							);
						});
					}
				}
		...
		return {
			...
			message:
				availabilityMessage ||
				`The next available time in the next ${lookaheadDays} days is shown above. ${timezoneNote}`,
		};
	} catch (error: any) {
...
async function bookMeeting(parameters: any) {
	...
	try {
		...
		if (!auth) {
			throw new Error(
				'Google Calendar API is not configured. Please set up Google service account environment variables.',
			);
		}
...
async function getCaseStudyChapter(parameters: any) {
	...
	try {
		...
		const requestedSection =
			sections.find(
				(section) =>
					section.id === chapterId || section.title.toLowerCase().includes(chapterId.toLowerCase()),
...
async function submitClientIntake(parameters: any) {
	...
	try {
		...
		if (!prisma) {
			throw new Error(
				'Database is not configured. Please set up Prisma and run database migrations.',
			);
		}
...
function generateAvailableSlots(
	...
	console.log(
		'🔍 Debug: Checking availability from',
		startDate.toISOString(),
		'to',
		endDate.toISOString(),
	);
	...
	busyTimes.forEach((busy, index) => {
		...
		const isAllDay =
			busyStart.getHours() === 0 &&
			busyStart.getMinutes() === 0 &&
			busyEnd.getHours() === 0 &&
			busyEnd.getMinutes() === 0;
	...
	while (currentDate < endDate) {
		console.log(
			`🔍 Debug: Processing date: ${currentDate.toISOString().split('T')[0]} (day ${currentDate.getDay()})`,
		);
		...
		for (const duration of MEETING_DURATIONS) {
			...
			const dayStart = new Date(
				`${dateStr}T${businessHours.start.toString().padStart(2, '0')}:00:00.000`,
			);
			const dayEnd = new Date(
				`${dateStr}T${businessHours.end.toString().padStart(2, '0')}:00:00.000`,
			);
			...
			const businessOffset = new Date(
				dayStart.toLocaleString('en-US', { timeZone: businessTimezone }),
			).getTimezoneOffset();
			const serverOffset = new Date(
				dayStart.toLocaleString('en-US', { timeZone: serverTimezone }),
			).getTimezoneOffset();
			...
			console.log(
				`🔍 Debug: Generating slots for ${dateStr} - ${adjustedDayStart.toISOString()} to ${adjustedDayEnd.toISOString()}`,
			);
			...
			while (slotStart < adjustedDayEnd) {
				...
				if (slotStart < minimumBookingTime) {
					console.log(
						`🔍 Debug: Skipping past slot: ${slotStart.toISOString()} (current time: ${now.toISOString()})`,
					);
				...
				const isAvailable = !busyTimes.some((busy) => {
					...
					if (isAllDay) {
						...
						const busyEndDate = new Date(busyEnd.getTime() - 24 * 60 * 60 * 1000)
							.toISOString()
							.split('T')[0]; // Subtract 1 day since end is next day midnight

						hasOverlap = slotDate >= busyStartDate && slotDate <= busyEndDate;

						if (hasOverlap) {
							console.log(
								`🔍 Debug: Slot ${slotStart.toISOString()} conflicts with ALL-DAY event ${busyStartDate} to ${busyEndDate}`,
							);
						}
					} else {
						// For regular events, check for time overlap
						hasOverlap =
							slotStartUTC < busyEnd.toISOString() && slotEndUTC > busyStart.toISOString();

						if (hasOverlap) {
							console.log(
								`🔍 Debug: Slot ${slotStart.toISOString()}-${slotEnd.toISOString()} conflicts with busy ${busyStart.toISOString()}-${busyEnd.toISOString()}`,
							);
						}
					}
					...
					if (slotStart.getHours() === 15 && slotStart.getMinutes() === 0) {
						console.log(
							`🔍 Debug: 3:00 PM slot check - Slot: ${slotStart.toISOString()}, Busy: ${busyStart.toISOString()}-${busyEnd.toISOString()}, AllDay: ${isAllDay}, Overlap: ${hasOverlap}`,
						);
					}
				...
				if (isAvailable && slotEnd <= adjustedDayEnd) {
					console.log(
						`✅ Debug: Available slot found: ${slotStart.toISOString()} - ${slotEnd.toISOString()}`,
					);

					// Additional debug for 2:00 PM slots specifically
					if (slotStart.getHours() === 14 && slotStart.getMinutes() === 0) {
						console.log(
							`✅ Debug: 2:00 PM slot marked as AVAILABLE: ${slotStart.toISOString()} - ${slotEnd.toISOString()}`,
						);
					}
					...
					if (slotStart.getHours() === 14 && slotStart.getMinutes() === 0) {
						console.log(
							`❌ Debug: 2:00 PM slot filtered out - Available: ${isAvailable}, Within day: ${slotEnd <= dayEnd}`,
						);
					}
				}
		...
		console.log(
			`🔍 Debug: Moving to next day: ${currentDate.toISOString().split('T')[0]} -> ${new Date(currentDate.getTime() + 24 * 60 * 60 * 1000).toISOString().split('T')[0]}`,
		);
		currentDate.setDate(currentDate.getDate() + 1);
	}
	...
	try {
		...
		return {
			...
			data: {
				message:
					message ||
					'To book a meeting, I need your contact information. Please provide your name and email address.',
...
async function processBookingRequest(parameters: any) {
	...
	try {
		...
		if (
			existingBookingResult.type === 'ui_action' &&
			existingBookingResult.action === 'show_existing_booking'
		) {
...
function filterSlotsForModal(
	...
	console.log(
		`🔍 Debug: Filtered ${slots.length} slots down to ${filteredSlots.length} slots for modal`,
	);
	...
	try {
		...
		if (filteredSlots.length > 0) {
			...
			filteredSlots.slice(0, 3).forEach((slot, index) => {
				const slotTime = new Date(slot.start);
				console.log(
					`  ${index + 1}. ${slot.start} -> ${slotTime.toLocaleString('en-US', { timeZone: timezone })}`,
				);
			});
		}
