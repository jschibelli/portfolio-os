...
async function fetchAllFilesFromRepo() {
  try {
    ...
    for (const path of possiblePaths) {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(files)) {
          ...
          console.log('🔍 Debug - Markdown files in', path, ':', markdownFiles.map((f: any) => `${f.name} (${f.updated_at})`));
          console.log('🔍 Debug - Other files in', path, ':', otherFiles.map((f: any) => `${f.name} (${f.type})`));
    ...
    debugInfo.markdownFiles = allFiles.filter((file: any) => 
      file.name.endsWith('.md') || file.name.endsWith('.markdown')
    ).length;
    ...
    return { allFiles: [], debugInfo: { error: error instanceof Error ? error.message : 'Unknown error' } };
  }
}
