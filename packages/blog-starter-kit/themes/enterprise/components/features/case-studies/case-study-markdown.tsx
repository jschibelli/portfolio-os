      ...
      <div ref={contentRef} className="hashnode-content-style mx-auto w-full px-5 md:max-w-screen-md">
        {parts.map((part, index) => {
          if (part.type === 'markdown') {
            ...
            // Handle enhanced block types
            // TODO: Re-enable when case-study-blocks module is available
            /*
            if (part.blockData.type === 'quote') {
              const { renderFencedBlocks } = require('../lib/case-study-blocks');
              const blocks = [{ 
                type: 'quote', 
                headers: [], 
                rows: Object.entries(part.blockData.data).map(([key, value]) => [key, value as string])
              }];
              const blockComponents = renderFencedBlocks(blocks);
              return <React.Fragment key={index}>{blockComponents}</React.Fragment>;
            } else if (part.blockData.type === 'timeline') {
              const { renderFencedBlocks } = require('../lib/case-study-blocks');
              const blocks = [{ 
                type: 'timeline', 
                headers: [], 
                rows: part.blockData.data.map((item: any) => [item.phase, item.title, item.duration, item.description])
              }];
              const blockComponents = renderFencedBlocks(blocks);
              return <React.Fragment key={index}>{blockComponents}</React.Fragment>;
            } else {
