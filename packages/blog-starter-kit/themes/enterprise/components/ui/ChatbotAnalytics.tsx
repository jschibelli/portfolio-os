...
interface AnalyticsEvent {
  type: 'conversation_start' | 'message_sent' | 'intent_detected' | 'action_clicked' | 'conversation_end';
  data: any;
  timestamp: string;
}
...
export default function ChatbotAnalytics() {
  ...
  const calculateAnalytics = (): ChatbotAnalytics => {
    try {
      const events: AnalyticsEvent[] = JSON.parse(localStorage.getItem('chatbot_analytics') || '[]');
  ...
  return (
    <div className="fixed top-4 left-4 z-[10000] bg-white dark:bg-stone-800 rounded-lg shadow-lg border border-stone-200 dark:border-stone-700 p-4 max-w-sm">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-sm">Chatbot Analytics</h3>
        <button
      ...
      <div className="space-y-3 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-stone-50 dark:bg-stone-700 p-2 rounded">
        ...
        {analytics.mostCommonIntents.length > 0 && (
          <div>
            <div className="font-medium mb-1">Top Intents</div>
        ...
        {analytics.popularActions.length > 0 && (
          <div>
            <div className="font-medium mb-1">Popular Actions</div>
            <div className="space-y-1">
