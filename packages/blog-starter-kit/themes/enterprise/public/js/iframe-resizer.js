...
!(function (e) {
	...
	function F(e) {
		...
		function c(e) {
			...
			return (
				O(L),
				{ x: Math.floor(Number(n.left) + Number(d.x)), y: Math.floor(Number(n.top) + Number(d.y)) }
			);
		}
		function u(e) {
		...
		'[iFrameResizerChild]Ready' === A
			...
			: a === ('' + A).substr(0, s) && A.substr(s).split(':')[0] in f
				? ((H = (k = A.substr(s).split(':'))[1] ? parseInt(k[1], 10) : 0),
					...
					!F &&
						((x = !0),
						f[(b = L)] || ((x = !1), I(B.type + ' No settings for ' + b + '. Message was: ' + A)),
						x) &&
						(y(L, 'Received: ' + A),
						(h = !0),
									...
									!(t.constructor === Array
										? (function () {
												var e = 0,
													n = !1;
												for (
													y(L, 'Checking connection is from allowed list of origins: ' + t);
													e < t.length;
													e++
												)
													if (t[e] === i) {
	...
	function H(n, o) {
		...
		function p(e) {
			1 / 0 !== f[g][e] &&
				0 !== f[g][e] &&
				((n.style[e] = f[g][e] + 'px'), y(g, 'Set ' + e + ' = ' + f[g][e] + 'px'));
		}
		function b(e) {
			...
			: ((l = (l = o) || {}),
				...
				(function () {
					switch (
						(y(
							g,
							'IFrame scrolling ' + (f[g] && f[g].scrolling ? 'enabled' : 'disabled') + ' for ' + g,
						),
