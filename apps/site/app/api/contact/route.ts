...
export async function POST(request: NextRequest) {
	try {
		...
		if (!features.email) {
			console.error(
				'📧 Email service not configured. Missing environment variables: RESEND_API_KEY or EMAIL_FROM',
			);
			return NextResponse.json(
				{
					...
					details:
						'The contact form requires email service configuration. Please ensure RESEND_API_KEY and EMAIL_FROM environment variables are set.',
				},
				{ status: 503 },
			);
		}
			...
			if (now < clientData.resetTime) {
				if (clientData.count >= RATE_LIMIT_MAX_REQUESTS) {
					return NextResponse.json(
						...
						{
							status: 429,
							headers: { 'Retry-After': Math.ceil((clientData.resetTime - now) / 1000).toString() },
						},
					);
				}
				clientData.count++;
		...
		try {
          ...
          <p><strong>Reason for reaching out:</strong> ${validatedData.projectType || 'Not specified'}</p>
...
Reason for reaching out: ${validatedData.projectType || 'Not specified'}
...
Client IP: ${clientIP}
        `,
			...
			const response: any = {
				...
				error: errorCode,
				message:
					"We've received your message but couldn't send the email notification. We'll follow up with you shortly. Alternatively, you can email john@schibelli.dev directly.",
