...
const GQL_ENDPOINT =
  (process.env.HASHNODE_GQL_ENDPOINT || process.env.NEXT_PUBLIC_HASHNODE_GQL_ENDPOINT || 'https://gql.hashnode.com').replace(
    /\/?$/,
    '/'
  );
...
// Circuit breaker: once Hashnode returns HTML/outage responses, skip further calls
// for this process lifetime so static builds don't thrash the API for every slug.
let hashnodeUnavailableUntil = 0;
const HASHNODE_CIRCUIT_OPEN_MS = 5 * 60 * 1000;
let hashnodeFailureLogged = false;

function isHashnodeCircuitOpen(): boolean {
  return Date.now() < hashnodeUnavailableUntil;
}

function openHashnodeCircuit(reason: string): void {
  if (!isHashnodeCircuitOpen()) {
    console.warn(
      `[Hashnode API] Opening circuit breaker for ${HASHNODE_CIRCUIT_OPEN_MS / 1000}s: ${reason}`
    );
  }
  hashnodeUnavailableUntil = Date.now() + HASHNODE_CIRCUIT_OPEN_MS;
}
...
function logHashnodeFailureOnce(error: unknown): void {
  if (hashnodeFailureLogged) return;
  hashnodeFailureLogged = true;
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[Hashnode API] Request failed (further identical failures suppressed): ${message}`);
}
...
async function makeGraphQLRequest(query: string, variables: Record<string, any>): Promise<any> {
  if (isHashnodeCircuitOpen()) {
    throw new Error('Hashnode circuit breaker is open (previous outage)');
  }
  ...
  try {
    ...
    } else if (process.env.NODE_ENV === 'production' && !hashnodeFailureLogged) {
      console.warn(
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      const timeoutError = new Error('Hashnode API request timed out');
      openHashnodeCircuit(timeoutError.message);
      logHashnodeFailureOnce(timeoutError);
      throw timeoutError;
    }
    if (isOutageError(error)) {
      openHashnodeCircuit(error instanceof Error ? error.message : String(error));
    }
    logHashnodeFailureOnce(error);
    throw error;
  }
}
