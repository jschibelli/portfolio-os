...
test.describe('Blog Post Detail Page - Core Rendering', () => {
	...
	test('should display post title in h1', async ({ page }) => {
		...
		await expect(h1).toHaveClass(/type-case-study-h1/);
	});
	...
	test('should render post content with proper HTML', async ({ page }) => {
		...
		const hasContent =
			(await contentContainer.locator('p, h1, h2, h3, h4, h5, h6, ul, ol, pre, code').count()) >
				0 || (await contentContainer.textContent().then((text) => text!.length > 0));
...
test.describe('Blog Post Detail Page - Metadata & UI', () => {
	test('should display author name with icon', async ({ page }) => {
		...
		const authorElement = page
			.locator('article header div:has-text("John Schibelli"), article header div.flex:has(svg)')
			.first();
	...
	test('should support dark mode styling', async ({ page }) => {
		...
		const themeToggle = page.locator(
			'button[aria-label*="theme"], button[aria-label*="dark"], button[aria-label*="light"], button[aria-label*="Switch to"]',
		);
		...
		if ((await themeToggle.count()) > 0) {
			...
			expect(
				articleClasses.includes('dark') ||
					contentClasses.includes('dark:bg-stone-800') ||
					contentClasses.includes('dark'),
			).toBeTruthy();
		}
	});
});
...
test.describe('Blog Post Detail Page - Interactive Elements', () => {
	...
	test('should not load chatbot component', async ({ page }) => {
		await page.goto('/blog', { waitUntil: 'domcontentloaded', timeout: 30000 });
		...
		const chatbotButton = page.locator(
			'button[aria-label*="chat"], button:has-text("Chat"), [data-testid*="chatbot"]',
		);
