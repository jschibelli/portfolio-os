    ...
    return (
      <li className="list-none">
        ...
        >
          ...
          <span className="ml-2">{(label ?? name) as any}</span>
        </div>
        <div
          className={cn(
        ...
        >
          <div className="overflow-hidden">
            <ul className="list-none !pl-[0.75rem]">
              {/* @ts-expect-error - React type mismatch between versions */}
              <ctx.Provider value={indent + 1}>{children as any}</ctx.Provider>
            </ul>
          </div>
        </div>
      </li>
...
export const File = memo(({ label, name }: FileProps) => (
  <li className="list-none">
    <div className="hover:text-muted-foreground inline-flex cursor-default items-center py-1 text-xs transition-all">
      ...
      <span className="mr-2 ml-2">{(label ?? name) as any}</span>
    </div>
  </li>
))
