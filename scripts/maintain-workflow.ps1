# Maintain Real-Time Workflow Script
# Usage: .\scripts\maintain-workflow.ps1 [-DryRun]
...
function Main {
    Write-ColorOutput "=== Maintain Real-Time Workflow ===" "Blue"
    ...
    foreach ($item in $items) {
        ...
        if ($content.type -eq "Issue") {
            ...
            elseif ($currentStatus -eq "Ready" -and -not $hasPRs) {
                Write-ColorOutput "Issue #$issueNumber - Ready but no PRs -> Backlog" "Yellow"
                $itemsToMove += @{
                    ItemId = $item.id
                    IssueNumber = $issueNumber
                    Title = $content.title
            ...
            elseif ($currentStatus -eq "In progress" -and $hasPRs) {
                Write-ColorOutput "Issue #$issueNumber - Has PRs -> Ready" "Yellow"
                $itemsToMove += @{
                    ItemId = $item.id
                    IssueNumber = $issueNumber
                    Title = $content.title
