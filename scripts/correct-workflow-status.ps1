# Correct Workflow Status Script
# Usage: .\scripts\correct-workflow-status.ps1 [-DryRun]
...
function Main {
    Write-ColorOutput "=== Correct Workflow Status ===" "Blue"
    ...
    foreach ($item in $items) {
        ...
        if ($content.type -eq "Issue") {
            ...
            elseif ($currentStatus -eq "In progress" -and $commitCount -eq 0) {
                Write-ColorOutput "  -> In progress but no commits, should move to Backlog" "Yellow"
                $itemsToMove += @{
                    ItemId = $item.id
                    IssueNumber = $issueNumber
                    Title = $content.title
            ...
            elseif ($currentStatus -eq "Backlog" -and $commitCount -gt 0 -and -not $hasPRs) {
                Write-ColorOutput "  -> Has commits but no PR, should move to In progress" "Yellow"
                $itemsToMove += @{
                    ItemId = $item.id
                    IssueNumber = $issueNumber
                    Title = $content.title
            ...
            elseif ($currentStatus -eq "In progress" -and $hasPRs) {
                Write-ColorOutput "  -> Has PRs, should move to Ready" "Yellow"
                $itemsToMove += @{
                    ItemId = $item.id
                    IssueNumber = $issueNumber
                    Title = $content.title
