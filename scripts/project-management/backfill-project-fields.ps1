...
function Get-SizeFromIssue {
    ...
    # Small tasks
    if ($title -match "fix|update|add.*link|docs.*link" -or $body -match "simple|quick|minor") {
        return $fieldMappings.Size.sOption
    }
    
    # Default to M as per memory
...
Write-Host "Errors: $errorCount issues" -ForegroundColor $(if ($errorCount -gt 0) { "Red" } else { "Green" })
Write-Host ""
