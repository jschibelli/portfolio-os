...
function Trigger-GitHubWorkflow {
    ...
    try {
        ...
        $result = gh api repos/jschibelli/portfolio-os/dispatches -X POST -f event_type="agent-status-update" -f client_payload="$webhookPayload"
...
$success = Trigger-GitHubWorkflow -IssueNumber $IssueNumber -Action $Action -AgentName $AgentName -DryRun:$DryRun
