# Configure All Launch Tuesday QA Issues Project Fields
...
"@
    ...
    $result = gh api graphql -f query=$mutation -f projectId=$ProjectId -f itemId=$projectItemId -f fieldId=$PriorityFieldId -f value=$issue.Priority
    if ($result -match '"updateProjectV2ItemFieldValue"') {
    ...
    # Configure Size
    $result = gh api graphql -f query=$mutation -f projectId=$ProjectId -f itemId=$projectItemId -f fieldId=$SizeFieldId -f value=$issue.Size
    if ($result -match '"updateProjectV2ItemFieldValue"') {
        Write-Host "  ✅ Size set"
    } else {
    ...
    # Configure App
    $result = gh api graphql -f query=$mutation -f projectId=$ProjectId -f itemId=$projectItemId -f fieldId=$AppFieldId -f value=$issue.App
    if ($result -match '"updateProjectV2ItemFieldValue"') {
        Write-Host "  ✅ App set"
    } else {
    ...
    # Configure Area
    $result = gh api graphql -f query=$mutation -f projectId=$ProjectId -f itemId=$projectItemId -f fieldId=$AreaFieldId -f value=$issue.Area
    if ($result -match '"updateProjectV2ItemFieldValue"') {
        Write-Host "  ✅ Area set"
    } else {
