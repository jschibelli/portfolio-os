# Smart Assignment Script
# Usage: .\scripts\smart-assignment.ps1 [-DryRun]
...
function Assign-Issue {
    ...
    try {
        if ($Assignee -eq "jschibelli") {
            ...
            # For now, assign to jschibelli if Copilot user doesn't exist
            Write-ColorOutput "  Note: Copilot user not found, assigning to jschibelli instead" "Yellow"
            gh issue edit $IssueNumber --add-assignee jschibelli
        }
...
function DetermineAssignee {
    ...
    if ($copilotScore -gt $humanScore) {
        ...
        # Default to jschibelli for unassigned issues (since Copilot user doesn't exist)
        return "jschibelli"
    }
}

function Main {
    Write-ColorOutput "=== Smart Assignment ===" "Blue"
