...
Thank you for highlighting the environment configuration concerns. We have implemented the following security measures:
...
We have addressed all code quality concerns:
...
Thank you for the comprehensive review. We have addressed all the points raised:
...
### Merge Commands
Step 1: GitHub Web - https://github.com/jschibelli/portfolio-os/pull/$PRNumber
Step 2: CLI - gh pr merge $PRNumber --rebase --delete-branch
...
### Next Steps
Step 1: Address the issues above
Step 2: Re-run this automation
Step 3: Proceed with merge when ready
"@
    }
}
...
try {
    ...
    Write-Host "`nConfiguring project fields..." -ForegroundColor Blue
    ...
    Write-Host "`nFound $($crgptComments.Count) CR-GPT comments" -ForegroundColor Yellow
    ...
    if ($crgptComments.Count -gt 0) {
        ...
        Write-Host "`nUpdating status to 'In review'..." -ForegroundColor Yellow
